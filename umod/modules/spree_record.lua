...
function checkSpreeRecordRunFrameEndRound(vars)
    if not game["endRoundTrigger"] then
        for i = 0, clientsLimit, 1 do
            if client[i]["nbKill"] > spree["killsRecord"] then
                writeSpreeRecord(i, client[i]["nbKill"])
            end

            if client[i]["nbKill"] > mapSpree["killsRecord"] then
                writeMapSpreeRecord(i, client[i]["nbKill"])
            end
        end
...
function loadSpreeRecord()
    ...
    if len == -1 then
        et.G_LogPrint("WARNING: sprees/spree_record.dat file no found / not readable!\n")
    elseif len == 0 then
    ...
    if len == -1 then
        et.G_LogPrint("WARNING: sprees/" .. mapName .. ".record file no found / not readable!\n")
    elseif len == 0 then
...
function checkSpreeRecordObituaryEnemyKill(vars)
    client[vars["killer"]]["nbKill"] = client[vars["killer"]]["nbKill"] + 1
end
