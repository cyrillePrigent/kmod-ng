...
function execute_command(params)
    ...
    if clientNum then
        if clientNum >= 0 and clientNum < 64 then
            if et.gentity_get(clientNum, "pers.connected") ~= 2 then
                if params.commandSaid then
                    et.trap_SendConsoleCommand(et.EXEC_APPEND, params.say .. " ^3Gib: ^7There is no client associated with this slot number\n")
                else
            ...
            if params.commandSaid then
                et.trap_SendConsoleCommand(et.EXEC_APPEND, params.say .. " ^3Gib: ^7Please enter a slot number between 0 and 63\n")
            else
        ...
        if params.client then
            ...
            if e <= 2 then
                if params.commandSaid then
                    et.trap_SendConsoleCommand(et.EXEC_APPEND, params.say .. " ^3Gib: ^7Player name requires more than 2 characters\n")
                else
        ...
        if not clientNum then
            if params.commandSaid then
                et.trap_SendConsoleCommand(et.EXEC_APPEND, params.say .. " ^3Gib: ^7Try name again or use slot number\n")
            else
