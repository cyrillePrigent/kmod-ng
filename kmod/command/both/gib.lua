...
function execute_command(params)
    if params.nbArg < 2 then
        printCmdMsg(params, "Gib", "Useage: gib \[partname/id#\]\n")
        return 1
    end

    local clientNum = client2id(tonumber(params["arg1"]), 'Gib', params)

    if clientNum ~= nil
        if client[clientNum]['team'] ~= 1 and client[clientNum]['team'] ~= 2 then
            printCmdMsg(params, "Gib", "Client is not actively playing\n")

            return 1
        end

        if et.gentity_get(clientNum, "health") <= 0 then
            printCmdMsg(params, "Gib", "Client is currently dead\n")
